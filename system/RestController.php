...
{
     /*
      * ActiveRecord model class. The classname to use in controller actions
      */
     public static $model;
     /**
      ...
      */
     public static $allowed_cors="GET,POST,PUT,PATCH";
     /**
      ...
      *
      * Right now it is dummy function that will always return false. Don't forget to override it!
      */
     ...
     public static function get($request)
     {
         ...
         $result = $model->select()->where($model->asArray())->exec();
         return ['code' => '200 OK','message'=>$result];
     }
     /*
      * Those are dummy functions as an example. You are not ought to override them.
      * But that certainly will appear more RESTful
     ...
     public static function view($request)
     {
         $model = new ($this->model)();
         $model->load($request->body);
         return ['code' => '200 OK','message'=>$model->load($model->select()->where($model->asArray())->exec())->asArray()];
     }
     /*
      * Those are dummy functions as an example. You are not ought to override them.
      * But that certainly will appear more RESTful
